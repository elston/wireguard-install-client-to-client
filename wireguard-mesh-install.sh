...
# ═══════════════════════════════════════════════════════════════════════════
#  WireGuard Mesh Installer — v2
# ═══════════════════════════════════════════════════════════════════════════
#
#  Интерактивный установщик WireGuard для объединения узлов в mesh-сеть
#  через публичный relay-узел. Роль (relay/nat) выбирается вручную и
#  не привязана к ОС.
#
#    • relay  — узел с публичным IP, принимает входящие подключения
#                и форвардит трафик между NAT-пирами (iptables FORWARD).
#    • nat    — узел за NAT без публичного IP, инициирует соединение
#                к relay и держит NAT-запись через PersistentKeepalive.
#
#  Особенности:
#    • трафик НЕ перенаправляется в интернет (нет redirect-gateway);
#    • в контейнерах без ядерного модуля wireguard автоматически ставится
#      BoringTun (userspace-реализация);
#    • после установки доступно меню управления (добавить/удалить пира,
#      обновить AllowedIPs, перезапуск, полное удаление);
#    • watchdog через cron каждые 2 минуты перезапускает wg-quick,
#      если последний handshake старше 120 секунд.
#
#  Поддерживаемые ОС:  Ubuntu 22.04+, AlmaLinux 9+, CentOS Stream 8+
#  Запуск:             sudo bash wireguard-mesh-install.sh
#
# ═══════════════════════════════════════════════════════════════════════════
#  ТИПОВЫЕ СЦЕНАРИИ
# ═══════════════════════════════════════════════════════════════════════════
#
#  ── Кейс 1. Два NAT-узла через один relay ────────────────────────────────
#
#        node-a (дом, за NAT)              node-b (офис, за NAT)
#        WG: 10.10.0.2                     WG: 10.10.0.3
#             │                                   │
#             │ handshake + keepalive 20s         │
#             ▼                                   ▼
#         ┌──────────────────────────────────────────────┐
#         │                   RELAY                      │
#         │           203.0.113.10 : 51820               │
#         │           WG адрес: 10.10.0.1/24             │
#         └──────────────────────────────────────────────┘
#
#       Пинг node-a → node-b идёт через relay:
#       10.10.0.2  ──►  10.10.0.1 (FORWARD)  ──►  10.10.0.3
#
#       relay:   peer(node-a) AllowedIPs = 10.10.0.2/32
#                peer(node-b) AllowedIPs = 10.10.0.3/32
#       node-a:  peer(relay)  AllowedIPs = 10.10.0.1/32, 10.10.0.3/32
#       node-b:  peer(relay)  AllowedIPs = 10.10.0.1/32, 10.10.0.2/32
#
#
#  ── Кейс 2. Hub-and-spoke: один relay + N NAT-узлов ──────────────────────
#
#                          ┌─────────────────┐
#                          │      RELAY      │
#                          │  203.0.113.10   │
...
#              (дом)     (офис)     (CI/CD)    (ноут)
#
#       Любой NAT-узел ↔ любой NAT-узел общаются через RELAY.
#       На каждом NAT-узле в AllowedIPs у relay-пира перечислены WG-адреса
#       всех остальных узлов (через поле «Дополнительные IP» при установке
#       или через меню «Обновить AllowedIPs»).
#
#
#  ── Кейс 3. Одиночный NAT-клиент → relay (без mesh) ──────────────────────
#
#          RELAY (edge)                       NAT (worker)
#          203.0.113.10 : 51820               за NAT
#          WG: 10.10.0.1                      WG: 10.10.0.2
#              ▲                                    │
#              │                                    │ handshake
#              │          WireGuard tunnel          │ + keepalive 20s
#              └────────────────────────────────────┘
#
#       Через wg0 доступен только relay (10.10.0.1).
#       Интернет-трафик идёт мимо wg0 (нет redirect-gateway).
#       Применимо для доступа к приватному API / БД / панели за edge-узлом.
#
# ═══════════════════════════════════════════════════════════════════════════
#
...
_peer_count() {
    local n
    n=$(grep -c '# BEGIN_PEER' /etc/wireguard/wg0.conf 2>/dev/null)
    echo "${n:-0}"
}
...
management_menu() {
    ...
    6)
        if ip link show wg0 &>/dev/null; then
            wg-quick down wg0 && ok "Остановлен." || err "Ошибка."
        else
            wg-quick up wg0 && ok "Запущен." || err "Ошибка."
        fi
        sleep 1; management_menu ;;
    7)
        install_watchdog
